...
$titular=$l['proprietario'];

//caso transferido..
$tr_prop=mysql_query("select * from pedigre_trocados where id_ped=$id and id_f= $id_f ");
$ftr=mysql_fetch_assoc($tr_prop);
$ntp=(int)mysql_num_rows($tr_prop);

if($ntp>0)$titular=$ftr['proprietario'];


$tr_prop_t=mysql_query("select * from pedigre_trocados_tarja where id_ped=$id and id_f= $id_f ");
$ftr_t=mysql_fetch_assoc($tr_prop_t);
$ntpt=(int)mysql_num_rows($tr_prop_t);

if($ntpt>0)$titular=$ftr_t['proprietario'];
...
if(is_file($filename)==TRUE)$pdf->Image($filename,14.5, 1.3 , 3 , 3,'PNG');
...
if(is_file($pt)==true)$pdf->Image($pt,0.6, 0.6 , 2.8 , 2.8);
...
$pdf->SetXY(5.4,(3.1+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(5.5, 0.8,iconv('UTF-8', 'windows-1252', strtoupper(trim($l['amigo']))) , 0 , 1 , 'L' );
...
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', mb_strtoupper(trim($raca.' '.$var), 'UTF-8')) , 0 , 1 , 'L' );

$pdf->SetXY(2.0,(4.5+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', strtoupper(trim($sexo))) , 0 , 1 , 'L' );


$pdf->SetXY(1.5,(4.9+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', strtoupper(trim($pai))) , 0 , 1 , 'L' );


$pdf->SetXY(1.7,(5.4+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', strtoupper(trim($mae))) , 0 , 1 , 'L' );


$pdf->SetXY(2.3,(5.9+$yy));
$pdf->SetFont('arial','',9);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', mb_strtoupper(trim($titular),"UTF-8")) , 0 , 1 , 'L' );



$pdf->SetXY(5.4,(4.5+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', strtoupper(str_replace('Escolha a cor..','',$cor))) , 0 , 1 , 'L' );
