...
//if($ncode<1|| $dcode['id_filhote']==0)die('<script>location=\'http://www.megapedigree.com/painel_geral_123/imprime_rgc/ver_carteirinha.php?code='.$_GET['code'].'\';</script>');
...
$qcota=mysql_query("select * from ped_vias2 where id_user=".$id." and i_filhote=".$pgs);

$nr=mysql_num_rows($qcota);

if($nr<1 && substr($linha_ped['registro'],0,4)!='RG/E' ){echo "<script>location='../site/ver_carteirinha_alkc.php?id=".$id.str_pad(($pgs),2,"0",STR_PAD_LEFT)."';</script>";die();}



//inicio
if(!isset($_GET['bt'])) $hi= ';overflow: hidden';;
echo "<html><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><style>@media print { .noprint {display: none !important;}}</style><body oncontextmenu='return false;' style='font-size: 8;height:22cm;width: 17cm;<?=$hi?>'><div style=\"background-image:url('http://www.megapedigree.com/MODELO_alkc4.jpg');background-repeat:no-repeat;background-size: 98% 98%;font-size:10px;overflow:hidden;width:29.0cm;height:21cm;margin:-4px;padding:0px\">";
...
if(substr($linha_ped['registro'],0,4)=='RGEO'||substr($linha_ped['registro'],0,4)=='RG/E')$linha_ped['nome']=$lista_nomes[$ele];
...
echo "<div class='noprint' style='position:absolute;left:16mm;top:67mm;width:55mm;height:10mm'>".strtoupper( $coratual)."</div>";
...
while($i<=4){
	...
	while($j<pow(2,$i)){
		...
		echo "<div class='noprint' style='position:absolute;left:".$offx."mm;top:".(75.0 +$offy[$i]+((95/pow(2,$i))+0.2)*$j)."mm;width:".$largs[$i]."mm;height:".(95/16)."mm;font-size:$tamanhoInicial px;'>".strtoupper(str_pad(str_replace(',','<br>',str_replace('digite o nome..','XXXXXXXXXXXXXXX',array_shift($p))),50,' '))."</div>";
		$j++;
	}
	if($tamanhoInicial>2)$tamanhoInicial--;
	$i++;
}
...
$q_serie=mysql_query("SELECT * FROM ped_serie_a where id_ped=$id and id_filhote=$pgs and tipo_serie=0 ");
...
position: absolute;
top: 0;
left: 0;'><span style='background: lightblue none repeat scroll 0% 0%;
width: 100px;
height: 28px;
display: block;
top: 690px;
