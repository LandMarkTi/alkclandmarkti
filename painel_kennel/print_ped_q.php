...
//271eb282bff261f9d36727d132b66569
//todo: inserir foto usando dados jpg na pasta sobraci para banco.

//if($ttl>1605000000)die('<p> </p><script>location="index_principal.php";</script>');
...
$password_conexao = "-";
$conexao = mysql_pconnect($hostname_conexao, $username_conexao, "Companheiro3@10") or die('');
...
$query1 = "select * from ped_serie_a where numero_serie='$str_serie' and status='usado' and tipo_serie='0' and id_credenciado='$idc' ";
$query2 = "select * from ped_serie_a where numero_serie='$str_serie' and status='livre' and tipo_serie='0' and id_credenciado='$idc' ";

$t_query=mysql_query($query1) or die('err_pedserie');
$nt=(int)mysql_num_rows($t_query);

$t_query2=mysql_query($query2) or die('err_pedserie2');
...
if($livre>=1&&$nt<1){
 ...
 $upp=mysql_query("update ped_serie_a set status='usado',id_ped='$id',id_filhote='$pgs[0]',data_add='".time()."' where numero_serie='$str_serie' and id_credenciado='$idc' and tipo_serie='0'");
...
if ($nr>0&&$fr['conta_via']>=4)die('<script>alert("Limite de impressão atingido!");history.go(-1);</script>');
if ($nr>0&&($fr['conta_via']<4))mysql_query('update ped_vias2 set conta_via=conta_via+1 where id_user='.$id." and i_filhote =".$value);else mysql_query("insert into ped_vias2 values('',$id,".time().",".$_SESSION['id'].",1,$value)");
}
...
$sql="SELECT * FROM `gabarito_crg2` where id_gab<16 and id_gab<>11 and id_gab<>7 and id_gab<>10";
$qr=mysql_query($sql);
...
$pdf->SetXY(204,64);
//34899
$idcc=(int)$linha_ped['id_criador'];
$q_cn=mysql_query("SELECT * FROM criador_cn WHERE id =".$idcc);
$ncn=mysql_num_rows($q_cn);
$fcn=mysql_fetch_assoc($q_cn); // comentario
if($ncn>=1)$pdf->MultiCell(60,5,iconv('UTF-8', 'windows-1252',$fcn['cn']),$border,'c');
...
$ff=(int)$ele;
$ff-=4;
$fileqr = 'temp/qrtest'.$id.$ff.'.png';
if(is_file($fileqr)==TRUE)$pdf->Image($fileqr,255,19,25,25,'PNG');
