...
$t_query=mysql_query("select * from ped_serie_a where numero_serie='$str_serie' and tipo_serie='0' and id_credenciado='102' and status='livre' ") or die('err_pedserie');
$nt=(int)mysql_num_rows($t_query);


if($nt>=1){
mysql_query("insert into ped_serie_a values('',102,$id,$pgs[0],$serie,'0','usado',".time().")");
mysql_query("delete from ped_serie_a where numero_serie='$str_serie' and tipo_serie='0' and status='livre'");
} else die('Serie duplicada'); 
...
while($i<=4){
	...
	while($j<pow(2,$i)){
		...
		$pdf->MultiCell($largs[$i],3,mb_strtoupper(str_pad(str_replace(',',"\n",str_replace('digite o nome..','XXXXXXXXXXXXXXX',array_shift($p))),50,' '),'ISO-8859-1'),$border,'L');//era 56
...
//era 159 na outra grafica, add 1 de tudo na frente do x menos frase
