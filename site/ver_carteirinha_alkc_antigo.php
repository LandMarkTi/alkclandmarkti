...
//erro offset filhote, criado no imprime_canil/tarja.php
$qcota=mysql_query("select * from ped_serie_a where id_ped=".$id_ped." and (id_filhote=".($id_f-4).' or id_filhote='.$id_f.' ) ') or die("<script>alert('nenhum registro encontrado.');history.go(-1);</script>");

$nr=mysql_num_rows($qcota);

if($nr>=1 || $id_ped<72703){} else die('registro sem validade');

//canil

$sqlc = "select * from criadores  where id_criador = $l[id_criador] ";
$queryc = mysql_query($sqlc) or die ('ec');
...
?>
<a class="bt_ver" href="pedcode.php?id_ped=<?=$l['id_ped']?>&id_filhote=<?=$id_f?>">Ver Pedigree</a>
