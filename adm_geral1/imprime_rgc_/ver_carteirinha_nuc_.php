...
if($kid!=$id)die('chave inválida');

$id_trans=$_POST['id_trans'];

$id_ped=substr($id_trans,0,-2);

$id_f=(int)substr($id_trans,-2,2);
...
mysql_query("insert into cart values('','$id_ped','$id_f','../../rgc/$fotonome')")or die('ee');
}

}

$sql = "select * from pedigre_trocados join pedigree using(id_ped) where id_ped = $id_ped and id_f = $id_f";
...
$qf=mysql_query('select * from cart where id_ped='.$id_ped.' and id_f='.$id_f) or die('dd');
...
<span id="RGC"><?php echo 'RG/TN '.$id; ?></span>
...
<div class="noPrint" style="display: inline; position: relative;   left: 334px;top: -40px;<?php if($_GET['view']=='web') echo 'display:none';?>"><b>Foto : </b><form style="display:inline" method="post"  enctype="multipart/form-data"><input type="hidden" name="id_trans" value="<?=$id?>"><input type="file"  name="foto"><input type="submit" class="link" value="enviar"></form></div>
</p>
</body>
</html>
