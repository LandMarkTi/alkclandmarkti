...
$t_query=mysql_query("select * from ped_serie_a where numero_serie='$str_serie' and status='usado' and tipo_serie='3'  ") or die('err_pedserie');
$nt=(int)mysql_num_rows($t_query);

$t_query2=mysql_query("select * from ped_serie_a where numero_serie='$str_serie' and status='livre' and tipo_serie='3'  ") or die('err_pedserie2');
...
if($livre>=1&&$nt<1){
 ...
 $upp=mysql_query("update ped_serie_a set status='usado',id_ped='$id',id_filhote='$id_f',data_add='".time()."' where numero_serie='$str_serie'  and tipo_serie='3' ");
...
//tranferidos
$qtar=mysql_query("select * from pedigre_trocados_tarja  where id_ped=".$id." and id_f=".$id_f);

$f_tar=mysql_fetch_assoc($qtar);

$prop=$f_tar['proprietario'];
...
$pdf->SetXY(2,(5.9+$yy));
$pdf->SetFont('arial','',8);
$pdf->Cell(9.5, 0.8,iconv('UTF-8', 'windows-1252', trim($prop)) , 0 , 1 , 'L' );
