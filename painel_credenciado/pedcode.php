...
echo "<html><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><body style='font-size: 9;'><style>@media print { .noprint {display: none !important;}}</style><div style=\"background-image:url('http://www.megapedigree.com/MODELO_PED.png');background-size: 101% 101%;width:21cm;height:30cm;margin:-19px;padding:0px\">";
...
if(substr($linha_ped['registro'],0,4)=='RGEO')$linha_ped['nome']=$lista_nomes[$ele];
...
echo "<div class='noprint' style='position:absolute;left:".$l['px']."mm;top:".$l['py']."mm;width:".$l['largura']."mm;height:".$l['altura']."mm'>".$linha_ped[$l['nome']]."</div>";
...
while($i<=4){
	...
	while($j<pow(2,$i)){
		...
		echo "<div class='noprint' style='position:absolute;left:".$offx."mm;top:".(105.5+((95/pow(2,$i))+0.2)*$j)."mm;width:".$largs[$i]."mm;height:".(95/16)."mm'>".strtoupper(str_pad(str_replace('digite o nome..','XXXXXXXXXXXXXXX',array_shift($p)),50,' '))."</div>";
		$j++;
	}
...
while($j<4){
		...
		echo "<div class='noprint' style='position:absolute;left:".$px."mm;top:".(79.0+6.0*$j)."mm;width:".$largs[$i+1]."mm;height:6mm'>".substr(str_replace('NOME FILHOTE','XXX',strtoupper(array_shift($n))),0,20)."</div>";
		$j++;}
...
//$pdf->MultiCell(195,5,iconv('UTF-8', 'windows-1252', str_replace("\n","",$linha_ped['regrasublocal'])),$border,'L');
		echo "<div class='noprint' style='position:absolute;left:7mm;top:205mm;width:195mm;height:6mm'>". str_replace("\n","",$linha_ped['regrasublocal'])."</div>";
