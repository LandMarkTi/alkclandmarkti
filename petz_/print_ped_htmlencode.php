...
$nloop=explode(';',utf8_decode($linha_ped['ninhada']));
...
$sx=explode(';',utf8_decode($linha_ped['sexo']));
$cor=explode(';',utf8_decode($linha_ped['cor']));

while($ele=array_shift($pgs)){
$coratual=utf8_decode($cor[$ele-4]);
$sexoatual=utf8_decode($sx[$ele-4]);

$pdf->AddPage();
$pdf->SetFont('Arial','B',8);

$linha_ped['nome']=utf8_decode($lista_nomes[$ele]);

if($sexoatual=='Macho')$linha_ped['genero']='MACHO'; else $linha_ped['genero']='FEMEA';
$linha_ped['variedade']=utf8_decode($coratual);
...
$pdf->SetXY($l['px'],$l['py']);
$pdf->Cell($l['largura'],$l['altura'],strtoupper(utf8_decode($linha_ped[$l['nome']])),$border,0,'c');

}
//cor 

$pdf->SetXY(83,39);
$pdf->Cell(55,10,strtoupper(utf8_decode($coratual)),$border,0,'c');
...
$p=explode(';',utf8_decode($linha_ped['parentes']));
...
$n=explode(';',utf8_decode($linha_ped['ninhada']));
//ajustes
$e1=array_shift($n);
$e2=array_shift($n);
$e3=array_shift($n);
...
$pdf->SetXY(7,205);
$pdf->MultiCell(195,5,utf8_decode(str_replace("\n","",$linha_ped['regrasublocal'])),$border,'L');
...
if($nnn>=1)$pdf->Image('./images/pedigree/'.$lin['arquivo'],8,67,32,32);
}
