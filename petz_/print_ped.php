...
if ($nr>0&&$fr['conta_via']>=5&&($_SESSION['id']!=44&&$_SESSION['id']!=43))die('<script>alert("Limite de impressão atingido!");history.go(-1);</script>');
if ($nr>0&&($fr['conta_via']<5||$_SESSION['id']==44||$_SESSION['id']==43))mysql_query('update ped_vias2 set conta_via=conta_via+1 where id_user='.$id." and i_filhote =".$value);else mysql_query("insert into ped_vias2 values('',$id,".time().",".$_SESSION['id'].",1,$value)");
}
