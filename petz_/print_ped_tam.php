...
$linha_ped['endereco']=substr($linha_ped['endereco'],0,42);
...
$pdf->SetXY($l['px'],$l['py']);
$pdf->Cell($l['largura'],$l['altura'],iconv('UTF-8', 'windows-1252', mb_strtoupper($linha_ped[$l['nome']], 'UTF-8')),$border,0,'c');
...
while($i<=4){
	...
	while($j<pow(2,$i)){
		...
		$pdf->MultiCell($largs[$i],95/16,mb_strtoupper(str_pad(str_replace('digite o nome..','XXXXXXXXXXXXXXX',array_shift($p)),50,' '),'ISO-8859-1'),$border,'L');//era 56
		$j++;
	}
...
while($j<4){
		...
		$pdf->Cell($largs[$i+1],5.0,substr(str_replace('NOME FILHOTE','',mb_strtoupper(array_shift($n),'ISO-8859-1')),0,20),$border,0,'c');
		$j++;}
...
$pdf->SetXY(7,205);
$pdf->MultiCell(195,5,iconv('UTF-8', 'windows-1252', substr(str_replace("\n","",$linha_ped['regrasublocal']),0,2300)),$border,'L');
