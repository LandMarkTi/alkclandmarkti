...
if ($nr>0&&$fr['conta_via']>=6)die('<script>alert("Limite de impressão atingido!");history.go(-1);</script>');
if ($nr>0&&$fr['conta_via']<6)mysql_query('update ped_vias2 set conta_via=conta_via+1 where id_user='.$id." and i_filhote =".$value);else mysql_query("insert into ped_vias2 values('',$id,".time().",".$_SESSION['id'].",1,$value)");
}
//ajuste cota
$subcota=sizeof($pgs);
