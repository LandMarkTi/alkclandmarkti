...
$sql1="select * from pedigree  join subcategoria on pedigree.id_raca=subcategoria.idSubcategoria left join  registro_anterior using(id_ped) where id_criador in (".$inn.")";
...
$nn=explode(';',$linha_ped['ninhada']);
echo '<option title="'.str_replace("\"","",$nn[$linha_ped['id_filhote']]).' '.$linha_ped['registro'].($linha_ped['id_filhote']-4).'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$linha_ped['registro'].($linha_ped['id_filhote']-4).'</option>';
}
?>
