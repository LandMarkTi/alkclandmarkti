...
$sql1="select *,pedigree.id_criador ,criadores.nome_completo as nc,criadores.uso_canil   from adiciona_filhote   join  pedigree  using(id_ped) join criadores ON pedigree.id_criador=criadores.id_criador  left join foto_laudos using(id_ped)  where (foto_laudos.resultado IS NULL or foto_laudos.resultado=8 )and adiciona_filhote.id_criador=".$id;
