...
if($fc['id_credenciado']>=125)$inn=$id;
...
while($i<20){
	...
	if(is_null($linha_ped['reg_filhotes']))echo '<option longdesc="'.$ss[$i-4].'"  alt="'.$i.'" title="'.$pref.$nn[$i].$sulf.' '.$linha_ped['registro'].($i-4).'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$nn[$i].' '.$linha_ped['registro'].($i-4).'</option>';
	else echo '<option longdesc="'.$ss[$i-4].'" alt="'.$i.'" title="'.$nn[$i].' '.$linha_ped['registro'].($i-4).'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$nn[$i].' '.$linha_ped['registro'].($i-4).'</option>';
