...
if($raca==298||$raca==351||$raca==352)$sql1="select * from pedigree  join subcategoria on pedigree.id_raca=subcategoria.idSubcategoria left join  registro_anterior using(id_ped) where id_criador in (".$inn.") and (pedigree.registro not like 'RI%' OR pedigree.registro like 'RIO%' ) and id_raca in (298,351,352) ";

if($raca==323||$raca==353||$raca==354||$raca==382)$sql1="select * from pedigree  join subcategoria on pedigree.id_raca=subcategoria.idSubcategoria left join  registro_anterior using(id_ped) where id_criador in (".$inn.") and (pedigree.registro not like 'RI%' OR pedigree.registro like 'RIO%' ) and id_raca in (323,353,354,382) ";
...
$nn=explode(';',$linha_ped['ninhada']);
$ss=explode(';',$linha_ped['sexo']);
if($ss[$linha_ped['id_filhote']-4]=='Masc')echo '<option longdesc="'.$ss[$linha_ped['id_filhote']-4].'" title="'.str_replace("\"","",$nn[$linha_ped['id_filhote']]).' '.$linha_ped['registro'].($linha_ped['id_filhote']-4).'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$nn[$linha_ped['id_filhote']].' '.$linha_ped['registro'].($linha_ped['id_filhote']-4).'</option>';
}
?>
