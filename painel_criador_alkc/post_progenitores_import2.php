...
while($i<20){
	...
	if(is_null($linha_ped['reg_filhotes']))echo '<option title="'.$pref.$nn[$i].$sulf.' '.$linha_ped['registro'].($i-4).'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$nn[$i].' '.$linha_ped['registro'].($i-4).'</option>';
