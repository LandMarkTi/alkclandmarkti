...
if($raca==323||$raca==353||$raca==354||$raca==382)$sql1="select * from pedigree  join subcategoria on pedigree.id_raca=subcategoria.idSubcategoria left join  registro_anterior using(id_ped) left join foto_laudos using(id_ped) where id_criador in (".$inn.") and (foto_laudos.resultado IS NULL or foto_laudos.resultado=8 ) and (pedigree.registro not like 'RI%' OR pedigree.registro like 'RIO%' ) and id_raca in (323,353,354,382) ";
...
if($ss[$linha_ped['id_filhote']-4]=='Masc')echo '<option title="'.$pref.str_replace("\"","",$nn[$linha_ped['id_filhote']]).$sulf.' '.$vl[$linha_ped['resultado']].' '.$linha_ped['registro'].($linha_ped['id_filhote']-4).' '.$mc.'" value="'.str_replace("\"","",$linha_ped['id_ped']).'">'.$pref.$nn[$linha_ped['id_filhote']].$sulf.' '.$linha_ped['registro'].($linha_ped['id_filhote']-4).'</option>';
