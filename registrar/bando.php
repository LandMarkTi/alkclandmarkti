...
$qr_t=mysql_query("select * from criadores_b where  id_credenciado=$idcredenciado and email='".$eteste."'")or die('ect');
$ft=mysql_fetch_assoc($qr_t);
$n=mysql_num_rows($qr_t);
if($n>=1)die("<script>alert('Email já cadastrado,se necessário recupere sua senha pela capa do site.');location='form_bando.php';</script>"); 

$dtn=explode('/',$_POST['Nascimento1']);

$ann=(int)$dtn[2];

if($ann>=2000)die('Somente para maiores de 18 anos.');
...
$sobre=addslashes(strip_tags($_POST['sobrenome']));
...
$uso=addslashes(strip_tags($_POST['face']));
...
$envio = mail('comando@lokos.org', "Novo Loko", "$mensagemHTML", $headers,'-rcontato@megapedigree.com');
...
<br>
<br>
<p>Obrigado por se cadastrar,entraremos em contato .</p><br><br>
...
<input type="hidden" name="itemDescription1" id="lista" value="Cadastro ImperioLiga <?php echo $nc;?>" />
...
<?php $_SESSION['pstipo']='FILIAÇÃO LOKOS';
