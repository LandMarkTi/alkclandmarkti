...
if(isset($_GET['crmv'])) {
$cr=trim($_GET['crmv']);
$q_vet=mysql_query("select * from med_vet2 where crmv_med='".$cr."'");
...
$fototipo = $_FILES['arquivo']['type'];
$fototamanho = $_FILES['arquivo']["size"];
$fototemp = $_FILES['arquivo']['tmp_name'];



$id_ped=(int)trim($_POST['id_ped']);
$apl=mktime ( '00', '00', '00', substr($_POST['aplicacao'],3,2), substr($_POST['aplicacao'],0,2), substr($_POST['aplicacao'],6,4) );
$revac=time();//mktime ( '00', '00', '00', substr($_POST['revacinar'],3,2), substr($_POST['revacinar'],0,2), substr($_POST['revacinar'],6,4) );
$med=addslashes($_POST['responsavel']);
$desc=addslashes($_POST['desc']);
$desc=addslashes($_POST['obs']);
$vac=addslashes($fotonome);
$crmv=(int)$_POST['cr'];

move_uploaded_file($fototemp,'../prontuario/'.$fotonome);

mysql_query("INSERT INTO `rgpet`.`prontuario` (`id_vac`, `id`, `data_cadastro`, `data_form`,`desc`, `Resp_tec`, `arquivo`, `id_med`,`obs`) VALUES (NULL, '$id_ped', '$apl', '$revac', '$desc','$med', '$vac', '$crmv','$obs');");

die("<script>location='rgc_lista_prontuario.php'</script>");
...
<H1 style="color:#428F25;opacity:1;font-size: 27px;">Novo Procedimento </h1>
<br>
RG PET: &nbsp;&nbsp;
                <input type="text" name="id_ped" id="nome" size="30"  required>
                <br><br>
		...
		<div class="br">
			<div class="mesma" style="position: relative;top: -22px;">
		        ...
		        Tipo:<br>
		        <textarea name="desc" cols="25" required></textarea>                                
		        </div>
		</div>
...
<br>Obs:
<textarea name="obs" cols="25" required></textarea>               
		</div>
...
location='rgc_add_prontuario.php?crmv='+p;
<?php
