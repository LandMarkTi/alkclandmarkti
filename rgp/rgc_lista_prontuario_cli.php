...
<?php
	$sql = "select * from prontuario where id=".$cli." order by data_cadastro asc";
	$qr_p = mysql_query($sql) or die ('-');


	$sql = "select * from exames where id=".$cli." order by data_cadastro asc";
	$qr_e = mysql_query($sql) or die ('-');
...
.mesma{ display:inline-block;width: 20%;}
.br{ display:block;}
...
<H1 style="color:#428F25;opacity:1;font-size: 27px;">Prontuário </h1>
<br>
...
<?php while($l=mysql_fetch_assoc($qr_p)){?>
		<div class="br">
			<div class="mesma" style="">
		        ...
		        </div><div class="mesma">Procedimento
		        </div><div class="mesma">
...
<?php while($l=mysql_fetch_assoc($qr_e)){?>
		<div class="br">
			<div class="mesma" style="">
		        
		        <?=date("d/m/Y",$l['data_cadastro'])?>               
		        </div><div class="mesma">Exame
		        </div><div class="mesma">
		        <?=$l['desc']?><a target="_new" href="../prontuario/<?=$l['arquivo']?>">(ver)</a>              
		        </div>
		</div>
